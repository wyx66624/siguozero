...
关闭时两个网络都删除整盘投影中的阵亡特征输入列，检查点同时保存并校验该变体。
...
| Policy 参数 | 整盘线性层包含 75 个阵亡输入列 | 整盘线性层删除这 75 列 |
| 布局模型 | 不变 | 不变 |
...
revision 20 的整盘单层编码中，开启态比关闭态多 `75 × 256 = 19,200` 个 Policy 参数；Critic 同样相差 19,200。差异来自整盘投影的阵亡输入列，Layout 参数量不受影响。

| 模型档位 | 开启态 Policy | 关闭态 Policy | Layout（两者相同） |
|---|---:|---:|---:|
| bootstrap | 26,692,354 | 26,673,154 | 8,887,296 |
| main | 140,036,866 | 140,017,666 | 17,292,288 |
| extended | 207,211,266 | 207,192,066 | 25,697,280 |
