...
> 四国部署仍只用 Policy/Layout，训练阶段另有 Critic。
...
胜负局双方奖励互为相反数；和棋时所有玩家都得到训练奖励 `-0.15`。连续 **70 步**没有吃子时立即和棋，默认无总步数上限。规则结果统计中的和棋仍为 `0`，评测计半分。PPO 将队伍胜负与共同和棋惩罚分开传播，详见[负奖励与倒计时](draw_penalty_countdown_zh.md)。KL 和熵是优化正则项。
...
以下保留 revision 21 的架构说明；当前 revision 24 在该五维输入后增加
`max(0, 70 - no_capture_plies)`，四国为 `Linear(6,128)`、二人为 `Linear(6,256)`。
主视角点编号转换为全盘二维坐标后，原五维
...
正式启动值为：Policy 全局 batch `128` 个完整锚点组；本机二人 main、1,001 token 的 learner 容量探针中 RTX 4090 可放入 microbatch `24`（峰值分配约 `19.46 GiB`），正式任务先请求 `24`，若真实局面碎片导致 OOM 则自动原子重试为 `12`。H100 80GB 从 `32` 起测，H200 141GB 从 `64` 起测，再以梯度累积补足。布局模型仍使用全局 batch `1,024`。冷启动以约 `1M` 个锚点、`8M` 条终局续局验收，主训练至少规划 `8M` 个锚点、`64M` 条续局；冲击顶尖人类按 `25.6M～76.8M` 个锚点、`0.205B～0.614B` 条续局、约 `200K～600K` learner updates 预留。该范围是容量规划，不是棋力保证；完整实测表见[三模式训练设置、资源需求与运行手册](training_resources_zh.md)。
...
outer update 数不是预先固定值。默认每 update 有 `128 × 4 × 2 = 1,024` 条终局续局；若实际平均剩余长度为 $\bar H$：

$$
N_{rollout}=\frac{3.0\times10^9}{\bar H},\qquad
N_{anchor}=\frac{N_{rollout}}{8},
$$
...
- 跑至少一百万次布局和十万盘随机对局，确保无非法动作、无信息泄漏、70 步无吃子严格判和。
...
- 同时报告胜/和/负、座位分解、平均步数和 70 步无吃子和棋比例，不能只报挑选后的胜局。
...
| 无意义拖和 | 70 步无吃子和棋率持续上升 | 增加剩余和棋步数输入；终局训练和棋奖励 -0.15，以独立评测判断效果 |
