...
- 默认连续 60 手没有攻击任何敌子时立即和棋；
- 任意战斗会把无交互计数器清零；
- 可配置 `max_plies`，达到最大手数时判和；
- 军旗导致的团队胜负优先于和棋计数。
...
联合动作掩码在四国固定长度为 `5624`，二人固定长度为 `1176`。两阶段起点掩码长度分别为 `121` 和 `56`，选定起点后的条件目标掩码固定长度分别为 `76` 和 `35`。掩码中为 `True` 的编号与对应的合法编号接口完全一致。
